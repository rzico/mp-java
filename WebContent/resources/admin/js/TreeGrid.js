...
TreeGrid = function(_config){
    ...
    drowRowData = function(_rows, _cols, _level, _pid){
        ...
        for(var i=0;i<_rows.length;i++){
            ...
            s += "<tr id='TR" + id + "' pid='" + ((_pid=="")?"":("TR"+_pid)) + "' opens='Y' data=\"" + TreeGrid.json2str(row) + "\" rowIndex='" + rownum++ + "'>";
            for(var j=0;j<_cols.length;j++){
                ...
                //单元格内容
                if(col.dataField=='image'){
                    if(col.handler){
                        s += "<img  src='" + (eval(col.handler + ".call(new Object(), row, col)") || "") + "' style='width: 50px;height: auto;' class='image_hand'>" + "</td>";
                    }else{
                        s += "<img  src='" + (row[col.dataField] || "") + "' style='width: 50px;height: auto;' class='image_hand'>" + "</td>";
                    }
                }else {
                    if(col.handler){
    ...
    init = function(){
        ...
        __root.find("img[folder='Y']").bind("click", function(){
            ...
            var isOpen = __root.find("#" + trid).attr("opens");
            isOpen = (isOpen == "Y") ? "N" : "Y";
            __root.find("#" + trid).attr("opens", isOpen);
    ...
    showSubs = function(_trid){
        var isOpen = __root.find("#" + _trid).attr("opens");
        if(isOpen == "Y"){
...
TreeGrid.json2str = function(obj){
    ...
    for(var ii in obj){
        if(typeof obj[ii] != 'object'){ //暂时不包括子数据
            arr.push(ii + ":" + fmt(obj[ii]));
        }
    }
