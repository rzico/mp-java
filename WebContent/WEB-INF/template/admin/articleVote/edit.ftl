...
<div class="page-container">
    <form action="" method="post" class="form form-horizontal" id="form-update">
        ...
        [#if data??]
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>IP：</label>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="${data.value}" placeholder="" id="value" name="value">
            </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
                [#if articles??]
				<select name="articleId" class="select" style="background-color: #FFFFFF">
                    [#list articles as article]
					<option[#if data.article?? && article.id == data.article.id] selected[/#if] value="${article.id}">${article.name}</option>
                    [/#list]
				</select>
                [/#if]
				</span>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
                [#if articleVoteOptions??]
				<select name="articleVoteOptionId" class="select" style="background-color: #FFFFFF">
                    [#list articleVoteOptions as articleVoteOption]
					<option[#if data.articleVoteOption?? && articleVoteOption.id == data.articleVoteOption.id] selected[/#if] value="${articleVoteOption.id}">${articleVoteOption.name}</option>
                    [/#list]
				</select>
                [/#if]
				</span>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
                [#if authors??]
				<select name="authorId" class="select" style="background-color: #FFFFFF">
                    [#list authors as author]
					<option[#if data.author?? && author.id == data.author.id] selected[/#if] value="${author.id}">${author.name}</option>
                    [/#list]
				</select>
                [/#if]
				</span>
            ...
            $(function(){
                ...
                $("#form-update").validate({
                    ...
                    submitHandler:function(form){
                        ...
                        $(form).ajaxSubmit({
                            type: 'post',
                            url: "${base}/admin/articleVote/update.jhtml" ,
                            beforeSend: function() {
