...
<div class="page-container">
    <form action="" method="post" class="form form-horizontal" id="form-update">
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">CouponCode：</label>
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">MemberCard：</label>
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
                [#if memberCards??]
				<select name="memberCardId" class="select" style="background-color: #FFFFFF">
                    [#list memberCards as memberCard]
					<option[#if data.memberCard?? && memberCard.id == data.memberCard.id] selected[/#if] value="${memberCard.id}">${memberCard.name}</option>
                    [/#list]
				</select>
                [/#if]
				</span>
            ...
            $(function(){
                ...
                $("#form-update").validate({
                    ...
                    submitHandler:function(form){
                        ...
                        $(form).ajaxSubmit({
                            type: 'post',
                            url: "${base}/admin/payBill/update.jhtml" ,
                            beforeSend: function() {
