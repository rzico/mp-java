...
<div class="page-container">
    <form action="" method="post" class="form form-horizontal" id="form-update">
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">Card：</label>
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
                [#if cards??]
				<select name="cardId" class="select" style="background-color: #FFFFFF">
                    [#list cards as card]
					<option[#if data.card?? && card.id == data.card.id] selected[/#if] value="${card.id}">${card.name}</option>
                    [/#list]
				</select>
                [/#if]
				</span>
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">PayBill：</label>
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
                [#if payBills??]
				<select name="payBillId" class="select" style="background-color: #FFFFFF">
                    [#list payBills as payBill]
					<option[#if data.payBill?? && payBill.id == data.payBill.id] selected[/#if] value="${payBill.id}">${payBill.name}</option>
                    [/#list]
				</select>
                [/#if]
				</span>
            ...
            $(function(){
                ...
                $("#form-update").validate({
                    ...
                    submitHandler:function(form){
                        ...
                        $(form).ajaxSubmit({
                            type: 'post',
                            url: "${base}/admin/cardBill/update.jhtml" ,
                            beforeSend: function() {
