...
<div class="pd-20">
    <table class="table">
        ...
        <tr>
            ...
            [#if occupation.createDate??]
            <td><span id="createDate">${occupation.createDate}</span></td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">修改日期：</th>
            [#if occupation.modifyDate??]
            <td><span id="modifyDate">${occupation.modifyDate}</span></td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">排序：</th>
            [#if occupation.orders??]
            <td>${occupation.orders}</td>
            [/#if]
        </tr>
        <tr>
        ...
        <tr>
            ...
            [#if types??]
                ...
                [#if status.id == occupation.status]
                    <td>${status.name}</td>
                [/#if]
                [/#list]
            [/#if]
