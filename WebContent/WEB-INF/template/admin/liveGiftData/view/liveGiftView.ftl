...
<div class="pd-20">
    <table class="table">
        ...
        <tr>
            ...
            [#if liveGift.createDate??]
            <td><span id="createDate">${liveGift.createDate}</span></td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">修改日期：</th>
            [#if liveGift.modifyDate??]
            <td><span id="modifyDate">${liveGift.modifyDate}</span></td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">Orders：</th>
            [#if liveGift.orders??]
            <td>${liveGift.orders}</td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">${TableComment}：</th>
            [#if] liveGift.deleted??]
            [#if liveGift.deleted == true]
        ...
        <tr>
            <th class="text-r" width="80">价值：</th>
            [#if liveGift.price??]
            <td>${liveGift.price}</td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">缩略图：</th>
            [#if liveGift.thumbnail??]
            <td>${liveGift.thumbnail}</td>
            [/#if]
        </tr>
