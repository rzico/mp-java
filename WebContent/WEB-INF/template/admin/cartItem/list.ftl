...
<head>
    ...
    <title>CartItem管理</title>
</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 管理员中心 <span
        class="c-gray en">&gt;</span> CartItem管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
<div class="page-container">
    <div class=""> 日期范围：
        ...
        <button type="submit" class="btn btn-success radius" id="" onclick="search();" name="">
            <i class="Hui-iconfont">&#xe665;</i> 搜CartItem
        </button>
    </div>
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
        <a href="javascript:;" onclick="delAll()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
		<a href="javascript:;" onclick="add('添加CartItem','add.jhtml','','510')" class="btn btn-primary radius"><i
                class="Hui-iconfont">&#xe600;</i> 添加CartItem</a></span></div>
    <div class="mt-20">
...
<script type="text/javascript">
    ...
    $(function () {
        $('.table').DataTable({
            ...
            "sAjaxSource": "${base}/admin/cartItem/list.jhtml",
            ...
            "aoColumnDefs": [
                ...
                },
                {
                    ...
                    "mRender": function (data, display, row) {
                        if(data != null){
                            return "<u style='cursor:pointer' class='text-primary' onclick=\"show('" + data.name + "','cartView.jhtml?id=" + data.id + "','1000" + data.id + "','360','400')\">" + data.name + "</u>";
                        }else{
                            return "";
                        }
                    }
                ...
                }, 
                {
                    ...
                    "mRender": function (data, display, row) {
                        if(data != null){
                            return "<a title='编辑' href='javascript:;' onclick=\"edit('编辑CartItem','edit.jhtml?id=" + data + "','200" + data + "','510')\" class=\"ml-5\" style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>" +
...
//   表格自适应屏幕
    ...
    /*删除全部*/
    function delAll(){
        var url = "${base}/admin/cartItem/delete.jhtml";
        var i = 0;
    ...
    function del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                ...
                url: '${base}/admin/cartItem/delete.jhtml',
                dataType: 'json',
                success: function (data) {
                    layer.close(load);
                    if (data.type == "success") {
