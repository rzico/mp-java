...
<div class="pd-20">
    <table class="table">
        ...
        <tr>
            ...
            [#if couponCode.createDate??]
            <td><span id="createDate">${couponCode.createDate}</span></td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">修改日期：</th>
            [#if couponCode.modifyDate??]
            <td><span id="modifyDate">${couponCode.modifyDate}</span></td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">号码：</th>
            [#if couponCode.code??]
        ...
        <tr>
            <th class="text-r" width="80">是否启用：</th>
            [#if couponCode.usedDate??]
            <td><span id="usedDate">${couponCode.usedDate}</span></td>
            [/#if]
        </tr>
        <tr>
            <th class="text-r" width="80">优惠券：</th>
            [#if types??]
                [#list coupons as coupon]
                [#if coupon.id == couponCode.coupon]
                    <td>${coupon.name}</td>
                [/#if]
                [/#list]
            [/#if]
        ...
        <tr>
            ...
            [#if types??]
                ...
                [#if member.id == couponCode.member]
                    <td>${member.name}</td>
                [/#if]
                [/#list]
            [/#if]
