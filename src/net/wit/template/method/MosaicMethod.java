 ...
 * @version 3.0
 */
@Component("mosaicMethod")
public class MosaicMethod implements TemplateMethodModel {
	...
	public Object exec(List arguments) throws TemplateModelException {
		if (arguments != null && !arguments.isEmpty() && arguments.get(0) != null && StringUtils.isNotEmpty(arguments.get(0).toString())) {
			...
			if (value.length()>(width+4)) {
			   return new SimpleScalar(abbreviate(value, width, ellipsis)+value.substring(value.length()-4));
			} else {
			   return new SimpleScalar(abbreviate(value, width, ellipsis));
			}
		}
		return null;
	}
