...
public class imageChange {
    ...
    public static final int QCODEMAXWIDETH = 143;
    //定位吗 最小宽度为3北 在给予3像素误差
    public static final int QCODEMINWIDETH = 11;
    //轮廓识别误差
    public static final int QCODEERROR = 5;
    ...
    public static void main(String args[]) {
        ...
        try {
            //处理第一张图片
            File file = new File("H:\\shudong\\shu\\treehole\\treeholeServer\\qqq.jpg");
...
//            ImageIO.write(img, "jpg", newfile);
            ...
            //处理第二张图片
            File erweima = new File("H:\\shudong\\shu\\treehole\\treeholeServer\\fff.jpg");
...
//            int[][] unitmap = image.zoom(newmap, twoimg);
            ...
            //生成图片
            File newfile = new File("../treeholeServer/tti.jpg");
        ...
        for (int j = 1; j < height - 1; j++) {
            for (int i = 1; i < width - 1; i++) {
                ...
                if (map[j][i] == 1) {
                    ...
                    int kong = 0;
                    Boolean type = false;
                    for (int k = 1; k < QCODEMAXWIDETH; k++) {
                        //轮廓偶尔内凹或者外凸的偏差
                        if (j + k < height) {
                            if (map[j + k][i] == 1) {
                                h11++;
                    ...
                    if (wsum < QCODEMINWIDETH || wsum > QCODEMAXWIDETH || hsum > QCODEMAXWIDETH || hsum < QCODEMINWIDETH) {
                        i += wsum;
                        continue;
                    }
                    int x = i, y = j;
        ...
        for (int y = 0; y < map.length; y++) {
            ...
            for (int x = 0; x < map[y].length; x++) {
                if (map[y][x] == 1) {
                    ...
                    //获取符合的列
                    if (width > QCODEMINWIDETH && width < QCODEMAXWIDETH) {
                        //二维码只能是最小二维码的3-9倍
                        int b = width / 7;
                        //匹配高度算法 和宽度 对比 取最优
                        int height = 0;
